...
def text_collate(batch):
    ...
    adapted_batch = {
        ...
        "dep": collate_tensors([torch.tensor(b[1]).long() for b in notnone_batches]),
        "dependentGloss": collate_tensors([torch.tensor(b[2]).long() for b in notnone_batches]),
        "governorGloss": collate_tensors([torch.tensor(b[3]).long() for b in notnone_batches]),
        'text_mask_id': collate_tensors([torch.tensor(b[4]).long() for b in notnone_batches]),
    }
    return adapted_batch
...
def a2m_collate(batch):
    ...
    maskbatchTensor = lengths_to_mask(lenbatchTensor, device=databatchTensor.device, max_len=60)
    adapted_batch = {
